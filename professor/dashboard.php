 ...
 * Professor Dashboard
 * ============================================================
 * Shows professor's borrowed books, request status,
 * and quick actions for requesting books.
 */
...
mysqli_stmt_bind_param($res, "i", $user_id);
mysqli_stmt_execute($res);
$approved_requests = mysqli_fetch_assoc(mysqli_stmt_get_result($res))['total'];
mysqli_stmt_close($res);

$res = mysqli_prepare($conn, "SELECT COUNT(*) as total FROM requests WHERE user_id = ? AND status = 'returned'");
mysqli_stmt_bind_param($res, "i", $user_id);
mysqli_stmt_execute($res);
$returned_books = mysqli_fetch_assoc(mysqli_stmt_get_result($res))['total'];
mysqli_stmt_close($res);

// Borrow limit check
$borrow_limit = PROFESSOR_BORROW_LIMIT;
$limit_reached = isBorrowLimitReached($conn, $user_id, 'professor');

// My active requests (not returned/rejected)
$my_requests = mysqli_prepare($conn, 
    "SELECT r.*, b.title as book_title, b.author as book_author, b.cover_image 
     FROM requests r 
...
<div class="page-header">
    ...
    <p>Professor Dashboard — Borrow limit: <?php echo $active_borrows; ?>/<?php echo $borrow_limit; ?> books</p>
</div>
...
<div class="btn-group mb-3">
    <a href="request_book.php" class="btn btn-primary">
        <i class="fas fa-hand-holding"></i> Request a Book
    </a>
    <a href="bulk_request.php" class="btn btn-success">
        <i class="fas fa-layer-group"></i> Bulk Request for Students
    </a>
</div>
    ...
    <div class="card-body">
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p>You don't have any pending or active book requests.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
