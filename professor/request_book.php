...
/**
 * ============================================================
 * Request Book (Professor)
 * ============================================================
 * Professors can browse available books and request them.
 * Includes AJAX live search and availability check.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['book_id'])) {
    ...
    // Check borrow limit
    if (isBorrowLimitReached($conn, $user_id, 'professor')) {
        $error = 'You have reached your borrow limit of ' . PROFESSOR_BORROW_LIMIT . ' books.';
    } else {
        ...
        if (!$book) {
            ...
            if (mysqli_num_rows($dup_result) > 0) {
                ...
                if (mysqli_stmt_execute($req)) {
                    $success = 'Book "' . htmlspecialchars($book['title']) . '" requested successfully!';
                } else {
...
<div class="page-header">
    ...
    <p>Browse and request books from the library collection</p>
</div>
